...
export class Board implements GameBoard{
    ...
    getFullLines(){
        let lines: number[] = this.blocks.map(row => row.reduce((x,y) => x * y,1));
        return lines;
    }
    ...
    add(p: Piece){
        console.log("ADD");
        console.log(`(${p.origin[0]},${p.origin[1]})`);
    ...
    isEmpty(blocks: number[][], rowIdx: number, offset: number): boolean{
        ...
        for(let i = 0; i<blocks.length; i++){
            for(let j = 0; j<blocks[0].length; j++){
                if(blocks[i][j] !== 0){
                    if(rowIdx+i >= Shared.NUM_ROWS || offset+j < 0 || offset+j >= Shared.NUM_COLS){
                        console.log("NOT EMPTY - boundaries");
                        return false;
                    }
                    if(this.blocks[rowIdx+i][offset+j] !== 0){
                        console.log("NOT EMPTY - blocks");
                        return false;
                    }
                }
            }
